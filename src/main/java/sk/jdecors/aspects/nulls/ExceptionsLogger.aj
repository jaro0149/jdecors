...
public class ExceptionsLogger {
    ...
    @Pointcut("callsToAllMethods() "
            + "&& @annotation(sk.jdecors.annotations.logging.LogException)")
    public void callsToLoggedMethods() {
    }

    @Pointcut(value = "inspectedContext() "
            + "&& callsToLoggedMethods()")
    public void callsToLoggedMethodsInContext() {
    }

    @AfterThrowing(value = "callsToLoggedMethodsInContext()",
            throwing = "exception",
            argNames = "exception,joinPoint")
